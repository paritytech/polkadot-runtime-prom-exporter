...
export class NominationPools extends CTimeScaleExporter {
    ...
    constructor(workerPath: string, registry: PromClient.Registry, withProm: boolean) {
        ...
        if (this.withTs) {
            ...
            this.poolIdSql = sequelize.define("runtime_nom_pools_pool_id", {
    ...
    async writeUnbondingBalance(time: number, myChain: string, unbondingBalance: number, withProm: boolean) {
    ...
    async writePendingRewards(time: number, myChain: string, pendingRewards: number, withProm: boolean) {
    ...
    async doWork(exporter: NominationPools, api: ApiPromise, indexBlock: number, chainName: string) {
        ...
        const PoolsDetail = await Promise.all((await apiAt.query.nominationPools.bondedPools.entries()).map(async ([key, bondedPool]) => {
            ...
            await exporter.writePoolId(timestamp, chainName.toString(), poolId.toNumber(), exporter.withProm);
            await exporter.writeMembers(timestamp, chainName.toString(), members.toNumber(), exporter.withProm);
            await exporter.writeTotalPoints(timestamp, chainName.toString(), totalPoints.toNumber(), exporter.withProm);
            await exporter.writeTotalBalance(timestamp, chainName.toString(), totalBalance.toNumber(), exporter.withProm);
            await exporter.writeUnbondingBalance(timestamp, chainName.toString(), unbondingBalance.toNumber(), exporter.withProm);
            await exporter.writePendingRewards(timestamp, chainName.toString(), pendingRewards.toNumber(), exporter.withProm);
