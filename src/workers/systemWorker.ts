...
export class System extends CTimeScaleExporter {
	...
	constructor(workerPath: string, registry: PromClient.Registry, withProm: boolean) {
		...
		if (this.withTs) {
			this.systemFinalizedSql = sequelize.define(
				'chain_finalized_number',
				{
			...
			this.systemWeightSql = sequelize.define(
				'runtime_weight',
				{
					time: { type: Sequelize.DATE, primaryKey: true },
					chain: { type: Sequelize.STRING, primaryKey: true },
			...
			this.systemBLockLenghtSql = sequelize.define(
				'runtime_block_length_bytes',
				{
					time: { type: Sequelize.DATE, primaryKey: true },
					chain: { type: Sequelize.STRING, primaryKey: true },
			...
			this.systemSpecVersionSql = sequelize.define(
	...
	async clean(
		...
		startingBlockTime: Date,
		endingBlockTime: Date
	) {
		...
		await super.cleanData(
			this.systemBLockLenghtSql,
			myChainName,
			startingBlockTime,
			endingBlockTime
		);
		await super.cleanData(
			this.systemSpecVersionSql,
			myChainName,
			startingBlockTime,
			endingBlockTime
		);
	...
	async doWork(exporter: System, api: ApiPromise, indexBlock: number, chainName: string) {
		...
		await exporter.writeFinalizedNumber(
			timestamp,
			chainName.toString(),
			indexBlock,
			exporter.withProm
		...
		await exporter.writeWeight(
			timestamp,
			chainName.toString(),
			weight.normal.toNumber(),
			'normal',
			exporter.withProm
		);
		await exporter.writeWeight(
