...
export class StakingMinerAccount extends CTimeScaleExporter {
	...
	constructor(workerPath: string, registry: PromClient.Registry, withProm: boolean) {
		...
		if (this.withTs) {
			this.stakingMinerBalanceASql = sequelize.define(
				'runtime_balance_a',
				{
			...
			this.stakingMinerBalanceBSql = sequelize.define(
				'runtime_balance_b',
				{
					time: { type: Sequelize.DATE, primaryKey: true },
					chain: { type: Sequelize.STRING, primaryKey: true },
	...
	async clean(myChainName: string, startingBlockTime: Date, endingBlockTime: Date) {
		await super.cleanData(
			this.stakingMinerBalanceASql,
			myChainName,
			startingBlockTime,
			endingBlockTime
		);
		await super.cleanData(
			this.stakingMinerBalanceBSql,
			myChainName,
			startingBlockTime,
			endingBlockTime
		);
	...
	async doWork(
		...
		try {
			...
			if (balanceA.data.free != null) {
				await exporter.writeA(
					timestamp,
					chainName.toString(),
					balanceA.data.free.toBn().div(decimals(api)).toNumber(),
					exporter.withProm
				);
			} else {
			...
			if (balanceB.data.free != null) {
				await exporter.writeB(
					timestamp,
					chainName.toString(),
					balanceB.data.free.toBn().div(decimals(api)).toNumber(),
					exporter.withProm
				);
			} else {
