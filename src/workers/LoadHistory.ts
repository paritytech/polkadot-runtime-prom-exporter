...
export async function loadHistoryFromApi(
	...
	try {
		...
		for (
			let indexBlock = startingBlock;
			indexBlock > blockLimit;
			indexBlock = indexBlock - distanceBB
		) {
			...
			if ((startingBlock - indexBlock) % 100 == 0 && startingBlock - indexBlock != 0) {
				logger.info(
					`processed ${startingBlock - indexBlock}/${
						startingBlock - blockLimit
					} blocks for exporter ${exporterName} chain ${chainName}`
				);
			}
		}
	} catch (error) {
