...
export class CTimeScaleExporter {
	...
	constructor(public wPath: string) {
		...
		if (connectionString != '') {
			this.exportersVersionsSql = sequelize.define(
				'exporters_versions',
				{
	...
	async launchWorkers(
		...
		const checkRecord = await this.findExporterRecord(
			this.chainName,
			exporterName,
			exporterVersion,
			startingBlock,
		...
		if (isMainThread) {
			for (let indexThread = 0; indexThread < threadsNumber; indexThread++) {
				...
				worker.on('message', (result) => {
					...
					logger.debug(
						`finished ${completedWorkers} worker(s) ${indexThread + 1}/${threadsNumber} of ${
							this.workerPath
						}, ${chainName}`
					);
	...
	async writeExporterRecord(
		tableSql: typeof Sequelize,
		myChainName: string,
		myExporter: string,
		myVersion: number,
		startingBlock: number,
		...
		await this.cleanPreviousExporterRecord(
			this.exportersVersionsSql,
			myChainName,
			myExporter,
			startingBlock,
	...
	async cleanPreviousExporterRecord(
		tableSql: typeof Sequelize,
		myChainName: string,
		myExporter: string,
		startingBlock: number,
		...
		try {
			...
			for (var i = 0; i < result.length; i++) {
				if (result[i]['chain'] === myChainName && result[i]['exporter'] === myExporter) {
					historyRecords.set(myExporter, [
						result[i]['startingblock'],
						result[i]['endingblock'],
						myVersion,
						result[i]['distancebb']
					]);
				}
