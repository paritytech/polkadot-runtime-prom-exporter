...
export class ElectionProviderMultiPhase extends CTimeScaleExporter {
    ...
    async writeQueuedSolution(time: number, myChain: string, scoreStake: number, score: string, withProm: boolean) {
        ...
        if (this.withProm) {
            this.multiPhaseQueuedSolutionScoreSql.set({ score: score, chain: myChain }, scoreStake);
        }
    }
    ...
    async clean(api: ApiPromise, myChain: string, startingBlockTime: Date, endingBlockTime: Date) {
        await super.cleanData(api, this.multiPhaseUnsignedSolutionSql, myChain, startingBlockTime, endingBlockTime)
        await super.cleanData(api, this.multiPhaseSignedSolutionSql, myChain, startingBlockTime, endingBlockTime)
        await super.cleanData(api, this.multiPhaseQueuedSolutionScoreSql, myChain, startingBlockTime, endingBlockTime)
        await super.cleanData(api, this.multiPhaseSnapshotSql, myChain, startingBlockTime, endingBlockTime)
    }
        ...
        try {
           ...
           for (let ext of signed_block.block.extrinsics) {
               ...
               if (ext.method.section.toLowerCase().includes("electionprovider") && ext.method.method === "submitUnsigned") {
                   ...
                   exporter.writeSignedSolution(timestamp, chainName.toString(), weight, 'weight', exporter.withProm);
                   exporter.writeSignedSolution(timestamp, chainName.toString(), length, 'length', exporter.withProm);
               ...
               if (ext.method.section.toLowerCase().includes("electionprovider") && ext.method.method === "submit") {
                   ...
                   exporter.writeUnsignedSolution(timestamp, chainName.toString(), weight, 'weight', exporter.withProm);
                   exporter.writeUnsignedSolution(timestamp, chainName.toString(), length, 'length', exporter.withProm);
           ...
           if (events.filter((ev) => ev.event.method.toString() == "SignedPhaseStarted").length > 0) {
               ...
               exporter.writeSnapshot(timestamp, chainName.toString(), size.toNumber(), exporter.withProm);
           ...
           if (events.filter((ev) => ev.event.method.toString() == "SolutionStored").length > 0) {
               ...
               exporter.writeQueuedSolution(timestamp, chainName.toString(), queued.unwrapOrDefault().score.minimalStake.div(decimals(api)).toNumber(), 'x1', exporter.withProm);
               exporter.writeQueuedSolution(timestamp, chainName.toString(), queued.unwrapOrDefault().score.sumStake.div(decimals(api)).toNumber(), 'x2', exporter.withProm);
   
           }

        } catch (error) {
            console.log('doWork electionProviderMultiPhaseWorker error for chain', chainName, error)
        }
    }
}
