...
export class ElectionProviderMultiPhase extends CTimeScaleExporter {
	...
	constructor(workerPath: string, registry: PromClient.Registry, withProm: boolean) {
		...
		if (this.withTs) {
			this.multiPhaseUnsignedSolutionSql = sequelize.define(
				'runtime_multi_phase_election_unsigned',
				{
			...
			this.multiPhaseSignedSolutionSql = sequelize.define(
				'runtime_multi_phase_election_signed',
				{
					time: { type: Sequelize.DATE, primaryKey: true },
					chain: { type: Sequelize.STRING, primaryKey: true },
					measure: { type: Sequelize.STRING, primaryKey: true },
			...
			this.multiPhaseQueuedSolutionScoreSql = sequelize.define(
				'runtime_multi_phase_election_score',
				{
					time: { type: Sequelize.DATE, primaryKey: true },
					chain: { type: Sequelize.STRING, primaryKey: true },
			...
			this.multiPhaseSnapshotSql = sequelize.define(
				'runtime_multi_phase_election_snapshot',
				{
					time: { type: Sequelize.DATE, primaryKey: true },
					chain: { type: Sequelize.STRING, primaryKey: true },
	...
	async clean(myChainName: string, startingBlockTime: Date, endingBlockTime: Date) {
		await super.cleanData(
			this.multiPhaseUnsignedSolutionSql,
			myChainName,
			startingBlockTime,
			endingBlockTime
		);
		await super.cleanData(
			this.multiPhaseSignedSolutionSql,
			myChainName,
			startingBlockTime,
			endingBlockTime
		);
		await super.cleanData(
			this.multiPhaseQueuedSolutionScoreSql,
			myChainName,
			startingBlockTime,
			endingBlockTime
		);
		await super.cleanData(
			this.multiPhaseSnapshotSql,
			myChainName,
			startingBlockTime,
			endingBlockTime
		);
	...
	async doWork(
		exporter: ElectionProviderMultiPhase,
		api: ApiPromise,
		indexBlock: number,
		chainName: string
	) {
		try {
			...
			for (let ext of signed_block.block.extrinsics) {
				if (
					ext.method.section.toLowerCase().includes('electionprovider') &&
					ext.method.method === 'submitUnsigned'
				) {
					...
					await exporter.writeSignedSolution(
						timestamp,
						chainName.toString(),
						weight,
						'weight',
				...
				if (
					ext.method.section.toLowerCase().includes('electionprovider') &&
					ext.method.method === 'submit'
				) {
					let length = ext.encodedLength;
					let weight = (await api.rpc.payment.queryInfo(ext.toHex())).weight.toNumber();
					logger.debug(`detected submit`);
					await exporter.writeUnsignedSolution(
						timestamp,
						chainName.toString(),
						weight,
						'weight',
			...
			if (events.filter((ev) => ev.event.method.toString() == 'SolutionStored').length > 0) {
				...
				await exporter.writeQueuedSolution(
					timestamp,
					chainName.toString(),
					queued.unwrapOrDefault().score.minimalStake.div(decimals(api)).toNumber(),
					'x1',
					exporter.withProm
				);
				await exporter.writeQueuedSolution(
					timestamp,
					chainName.toString(),
					queued.unwrapOrDefault().score.sumStake.div(decimals(api)).toNumber(),
					'x2',
					exporter.withProm
				);
			}
			...
			logger.debug(
				`doWork electionProviderMultiPhaseWorker error for chain ${chainName}, ${error}`
			);
		}
	}
}
