...
export class TransactionPayments extends CTimeScaleExporter {
	...
	constructor(workerPath: string, registry: PromClient.Registry, withProm: boolean) {
		...
		if (this.withTs) {
			this.weightToFeeMultiplierSql = sequelize.define(
				'runtime_weight_to_fee_multiplier',
				{
	...
	async doWork(
		...
		const weightFreeMultFormated = weightFeeMultiplier
			.mul(new BN(100))
