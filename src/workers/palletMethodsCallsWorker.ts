...
export class PalletMethods extends CTimeScaleExporter {
	...
	constructor(workerPath: string, registry: PromClient.Registry, withProm: boolean) {
		...
		if (this.withTs) {
			this.palletsMethodsCallsSql = sequelize.define(
				'pallets_methods_calls',
				{
	...
	async write(
		...
		if (this.withProm) {
			this.methodsCallsMetric.set(
				{ section: mySection, type: myIsSigned, method: myMethod, chain: myChain },
				myCalls
			);
		}
	}
	...
	async doWork(exporter: PalletMethods, api: ApiPromise, indexBlock: number, chainName: string) {
		...
		signedBlock.block.extrinsics.forEach((ex, index) => {
			...
			const [count, issigned, mymethod]: [number, boolean, string] = sectionMethods.get(
				sectionMethod
			) || [0, isSigned, method];
