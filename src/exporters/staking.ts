...
class StakingExporter extends Staking implements Exporter {
	...
	constructor(registry: PromClient.Registry) {
		...
		this.exporterIdentifier = 'staking';
	...
	async perDay(api: ApiPromise, chainName: string) {
		...
		let totalBondedStake = ledgers
			.map(([_, ledger]) => ledger.unwrapOrDefault().total.toBn().div(decimals(api)).toNumber())
			.reduce((prev, next) => prev + next);
		let totalUnbondingChunks = ledgers
			.map(([_, ledger]) => {
				if (ledger.unwrapOrDefault().unlocking.length) {
					return ledger
						.unwrapOrDefault()
						.unlocking.map((unlocking) => unlocking.value.toBn().div(decimals(api)).toNumber())
						.reduce((prev, next) => prev + next);
				} else {
					return 0;
				}
	...
	async stakingHourly(baseApi: ApiPromise, chainName: string) {
		...
		let totalSelfStake = exposures
			.map(([_, expo]) => expo.own.toBn().div(decimals(api)).toNumber())
			.reduce((prev, next) => prev + next);
		let totalOtherStake = exposures
			.map(([_, expo]) => expo.total.toBn().sub(expo.own.toBn()).div(decimals(api)).toNumber())
			.reduce((prev, next) => prev + next);
		...
		let totalExposedNominators = exposures
			.map(([_, expo]) => expo.others.length)
			.reduce((prev, next) => prev + next);
	...
	async voterBags(baseApi: ApiPromise, chainName: string) {
		...
		async function correctWeightOf(
			node: PalletBagsListListNode,
			api: ApiDecoration<'promise'>
		): Promise<BN> {
		...
		if (api.query['bagsList']) {
			...
			const bagThresholds = api.consts.bagsList.bagThresholds.map((x) =>
				baseApi.createType('Balance', x)
			);
